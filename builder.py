from automata import NDA, DA, EPSILON
...
def primitive_nfa(actual_string):
...
def create_nfa(regexp):
    ...
    for c in regexp:
        if c in list(operations.keys()) + ['(', ')']:
            if buffer != '':
                automata_stack.append(primitive_nfa(buffer))
            buffer = ''
        if c in operations:
...
def convert_to_dfa(nfa):
    links = []
    newStates = [set(nfa.eps_close(0))]
    visitedStates = []
    alphabet = [x for x in list(nfa.table.keys()) if x != EPSILON]
    while len(newStates) > 0:
        ...
        for char in alphabet:
            newTmp = set(nfa.forward(tmp, char))
            if len(newTmp) != 0:
    ...
    old_final = set(nfa.final_states)
...
def minimize_dfa(dfa):
    ...
    sets = [{*dfa.final_states}]
    non_final = {*list(range(dfa.num_of_states()))}.difference(dfa.final_states)
    if len(non_final) > 0:
    ...
    new_table = {k: [None] * num_of_states for k in dfa.alphabet()}
    for i, s in enumerate(sets):
        for v in s:
            for c in dfa.alphabet():
                new_indexes = [sets.index(s) for s in sets if dfa.table[c][v] in s]
                new_table[c][i] = None if len(new_indexes) == 0 else new_indexes[0]
    new_final = [sets.index(s) for s in sets if s.intersection(dfa.final_states)]
    return DA(table=new_table, final_states=new_final)
